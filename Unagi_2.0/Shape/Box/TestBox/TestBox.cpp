...
Shape::TestBox::TestBox(Three_D::Camera a_Cam):
Mesh(),
shader(
...
void Shape::TestBox::InitMesh()
{
    SetVertices();
    SetNormal();
    SetIndices();
    numOfVertex = rendererMeshData.m_VertPos.size();
}


void Shape::TestBox::Draw()
{
    shader.Use();
...
void Shape::TestBox::SetPosition(const Vector3 & pos)
{
    trans.SetPosition(pos);
}

void Shape::TestBox::SetScale(const Vector3 & a_scale)
{
    trans.SetScale(a_scale);
}

void Shape::TestBox::SetRotation(const Vector3 & rot)
{
    trans.SetRotation(rot);
}

void Shape::TestBox::SetData()
{
  ...
  /*  glBufferData(GL_ARRAY_BUFFER, sizeof(float)*(vecPos.size()) * 3+(sizeof(float)*vecNom.size()*3), nullptr, GL_STATIC_DRAW);
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(float)*numOfVertex * 3, &rendererMeshData.m_VertPos[0]);
    glBufferSubData(GL_ARRAY_BUFFER, sizeof(float)*numOfVertex * 3, sizeof(float)*numOfVertex * 3, &rendererMeshData.m_VertNorm[0]);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(int)*numOfVertex, &rendererMeshData.m_Indices[0], GL_STATIC_DRAW);
...
void Shape::TestBox::SetVertices()
{
    float X = 0.2f;
    float Y = 0.2f;
    float Z = 0.5f;
    ...
    //vecPos = vecPos1;
    rendererMeshData.m_VertPos = vecPos1;
}

void Shape::TestBox::SetNormal()
{
    ...
    //vecNom = vertNom1;
    rendererMeshData.m_VertNorm = vertNom1;
}

void Shape::TestBox::SetIndices()
{
    ...
    //indices = indices1;
    rendererMeshData.m_Indices = indices1;
}

void Shape::TestBox::DeleteBufferData()
{
